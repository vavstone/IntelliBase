"""Инструменты ReAct агента: реализации + описания в JSON Schema.

Здесь живут три инструмента, словарь-allowlist `DISPATCH` и список `TOOLS`
с описаниями в формате OpenAI Chat Completions. Агент (`agent_react.run_react_with_reflection`)
...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "фактические данные о предметной области ФТС. Возвращает словарь "
                "{answer, top_score, sources[id,file_name,page,score,snippet], confident}. "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Рассчитывает текущие дату и время. Вызывай, когда нужно знать текущее время. "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Вызывай только для финального ответа клиенту и только после того, "
                "как все нужные данные уже собраны. Возвращает строку вида "
                "'Сообщение отправлено в {chat_id}'. "
